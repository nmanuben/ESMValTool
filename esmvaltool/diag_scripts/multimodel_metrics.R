...
units <- attr(data, 'Variables')$common[[var0]]$units
...
model_dim <- which(names(dim(data)) == "model")
if (metric == "agreement") {
  agreement <- AnoAgree(data, members_dim = model_dim)
  ...
  ArrayToNetCDF(list(variable = agreement, lat = lat, lon = lon, time = time), 
                paste0(plot_dir, var0, "_anomaly_agreement", "_", months, "_",model_names_filename, "_", start_projection, "_", end_projection, "_", start_reference, "_", end_reference, ".nc"))
  
  #Compute and plot multi-year mean
  time_dim <- which(names(dim(data)) == "time")
  data <- Mean1Dim(data,  time_dim)
  model_dim <- which(names(dim(data)) == "model")
  agreement <- AnoAgree(data, members_dim = model_dim)
  ...
  PlotEquiMap(drop(Mean1Dim(data, model_dim)), lat = lat, lon = lon, brks = brks, units =units, toptitle = title, filled.continents = FALSE,
              dots = drop(agreement) >= agreement_threshold,
              fileout = paste0(plot_dir, '/anomaly_map_stipling.png'))
}
